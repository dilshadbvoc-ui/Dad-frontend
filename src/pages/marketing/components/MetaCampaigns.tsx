...
import { getAccountInsights, type AdInsight } from "@/services/marketingService"
...
export function MetaCampaigns() {
  ...
  const { data: insightsData, isLoading: insightsLoading } = useQuery({
    queryKey: ['meta-account-insights'],
    queryFn: () => getAccountInsights(),
    enabled: !!isConnected,
  });

  const insights: AdInsight | undefined = Array.isArray(insightsData)
    ? insightsData[0]
    : insightsData?.insights?.[0] || (insightsData && !insightsData.error ? insightsData : undefined)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={`border ${isConnected ? 'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900' : 'bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border-blue-200 dark:border-blue-900'}`}>
        <CardContent className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Button
            ...
            size="sm"
            className={`shrink-0 ${isConnected ? "gap-2 border-green-200 text-green-700 hover:text-green-800" : "gap-2 bg-[#1877F2] hover:bg-[#166fe5]"}`}
      ...
      {/* Platform Stats — real numbers from the connected ad account, not shown at all when disconnected */}
      {isConnected && (
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xl font-bold">{insightsLoading ? '—' : formatNumber(insights?.reach)}</span>
              </div>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xl font-bold">{insightsLoading ? '—' : formatNumber(insights?.impressions)}</span>
              </div>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xl font-bold">{insightsLoading ? '—' : formatNumber(insights?.clicks)}</span>
              </div>
            </CardContent>
          </Card>
        </div>
