...
export const getLeadHealth = async (filters?: { branchId?: string; startDate?: string; endDate?: string }): Promise<LeadHealth> => {
    try {
        const response = await api.get('/analytics/lead-health', { params: filters });
