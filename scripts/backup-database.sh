...
# Database Backup Script for MERN CRM
# This script creates automated backups of the PostgreSQL database

# Configuration
BACKUP_DIR="./backups"
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="$BACKUP_DIR/mern_crm_backup_$DATE.sql"
RETENTION_DAYS=30
...
if [ $? -eq 0 ]; then
    ...
    find "$BACKUP_DIR" -name "mern_crm_backup_*.sql.gz" -type f -mtime +$RETENTION_DAYS -delete
    find "$BACKUP_DIR" -name "mern_crm_backup_*.sql" -type f -mtime +$RETENTION_DAYS -delete
    
    # List current backups
    echo ""
    echo "Current backups:"
    ls -lh "$BACKUP_DIR"/mern_crm_backup_* 2>/dev/null || echo "No backups found"
